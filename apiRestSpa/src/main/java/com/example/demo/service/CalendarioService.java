...
public class CalendarioService {
    ...
    private LocalDate tercerLunes(int year, int mes) {
        ...
        while (true) {
            if (date.getDayOfWeek() == DayOfWeek.MONDAY) count++;
    ...
    public boolean esDiaCerrado(LocalDate fecha) {
        ...
        if (FERIADOS_FIJOS.contains(MonthDay.from(fecha))) return true;

        if (fecha.equals(primerLunes(year, 2))) return true;
        if (fecha.equals(tercerLunes(year, 3))) return true;
        if (fecha.equals(tercerLunes(year, 11))) return true;
